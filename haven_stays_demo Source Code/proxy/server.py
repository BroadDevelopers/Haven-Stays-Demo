#!/usr/bin/env python3
"""Backend-for-frontend (BFF) proxy for the Haven Stays demo.
...
and does three things:
  ...
  1. On startup, ensures the Haven agent exists (creates it from
     agent/agent.py if absent) — no manual setup step.
...
logger = logging.getLogger("haven-proxy")

_HERE = Path(__file__).resolve().parent
_ROOT = _HERE.parent  # haven_stays_demo/
...
async def _ensure_agent() -> str:
    """Create the Haven agent if it doesn't already exist; return its id.
...
app = FastAPI(title="Haven Stays Demo BFF Proxy")
...
async def _startup() -> None:
    ...
    logger.info("Haven proxy ready — upstream=%s model=%s agent=%s", BASE_URL, MODEL, _AGENT_ID)
